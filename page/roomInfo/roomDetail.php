...
async function controlDoor(action, classroomId, id) {
    try {
        ...
        function applySort() {
            const table = document.querySelector(".log-container tbody");
            const rows = Array.from(table.rows);
            const criteria = parseInt(document.getElementById("sortCriteria").value, 10);
            const order = document.getElementById("sortOrder").value; // 'asc' or 'desc'

            rows.sort((a, b) => {
                let aText = a.cells[criteria].innerText.trim();
                let bText = b.cells[criteria].innerText.trim();

                if (criteria === 0) {
                    // 開閉日時のソート
                    aText = parseCustomDate(aText);
                    bText = parseCustomDate(bText);
        ...
        function parseCustomDate(dateStr) {
            // "YYYY年M月D日G時i分s秒" をパースする
            const parts = dateStr.match(/(\d{4})年(\d{1,2})月(\d{1,2})日(\d{1,2})時(\d{2})分(\d{2})秒/);
            if (parts) {
                return new Date(parts[1], parts[2] - 1, parts[3], parts[4], parts[5], parts[6]);
            }
            return new Date(); // 失敗時は現在日時を返す
        }
    </script>
                ...
                <div class="container">
                    <!-- <div class="classroom-status">
                        ...
                        <div class="status-container">
                            <div class="status-header">
                                ...
                                <h2><?= htmlspecialchars($key['classroomName'], ENT_QUOTES, 'UTF-8') ?>号室</h2>
                            </div>
                            <!-- <div class="status-details">
                                <p>現在の状況: <strong>
                                    < ?php
                                        if ($key['charStatus'] === "開") {
                                            echo "開";
                                        } elseif ($key['charStatus'] === "閉") {
                                            echo "閉";
                                        } else {
                                            echo htmlspecialchars($key['charStatus'], ENT_QUOTES, 'UTF-8'); // 他の値があればそのまま表示
                                        }
                                    ?>
                                </strong></p>
                            </div> -->
                            ...
                            <p id="timestamp">最終更新時間: 
                                <?= $timestamp ? $timestamp->format('Y年m月d日 H時i分s秒') : 'データがありません' ?>
                            </p>
                        ...
                        <!-- <div class="tag">
                            <label for="historyDate">日付選択:</label>
                            <input type="date" id="historyDate">
                            <button onclick="filterByDate()">履歴を表示</button>
                         </div> -->
                         <!-- <div class="tag">
                            <label for="historyDate">日付選択:</label>
                            <input type="date" id="historyDate">
                        ...
                        <div class="tag">
                            ...
                            <select id="sortCriteria">
                                <option value="0">開閉日時</option>
                                <option value="1">開閉判定</option>
                                <option value="2">開閉者氏名</option>
                            </select>
                            ...
                            <button onclick="applySort()">ソート</button>
                        </div>
                    ...
                    <div id="overlay" class="overlay">
                        <div class="overlay-content">
                            <h3><?= $key['classroomName'] ?>教室を開閉しますか</h3>
                    ...
                    <div class="log--container-wrapper">
                        <table class="log-container">
                            <thead>
                                <tr>
                                    <th onclick="sortTable(0)">開閉日時</th>
                                    <th onclick="sortTable(1)">開閉判定</th>
                                    <th onclick="sortTable(2)">開閉者氏名</th>
                                </tr>
                            </thead>
                            <tbody id="logTableBody">
                                <?php foreach ($keys as $key) { ?>
                                    <tr class="history">
                                        <td><?= htmlspecialchars($key['formatted_date']) ?></td>
                                        <td><?= htmlspecialchars(getResponse($key['openCloseHistry'])) ?></td>
...
<!-- <script>
    function filterByDate() {
        ...
        // リクエストを送信して履歴をフィルタリング
        if (selectedDate) {
            // AJAXリクエストを使用して選択された日付でフィルタリング
            const xhr = new XMLHttpRequest();
            xhr.open("GET", "history.php?date=" + selectedDate, true);
            xhr.onload = function() {
                if (xhr.status === 200) {
                    document.getElementById("logTableBody").innerHTML = xhr.responseText;
                }
            };
            xhr.send();
        } else {
